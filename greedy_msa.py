...
# Create scoring function for sequence alignment
scoring = get_scoring_function(alphabet, 1, -1, -1, -1)
...
for aln in aligned_inputs:
    print_alignment('Intermediate Pairs: ', aln, scoring)
...
print_alignment('Most Similar Pair: ', most_similar_pair, scoring)
...
print_alignment('Next Pair: ', next_best_pair, scoring)
...
for aln in seq_profile_aligns:
    print_alignment('Intermediate sequence-profile alignments', aln, scoring)
...
for aln in final_alns:
    print_alignment('Final sequence-profile alignment', aln, scoring)

# Align the two profiles
final_aln = align_profile_profile(alignments[0], next_best_pair[1], scoring, alphabet)
print_alignment('Final Profile-Profile alignment', final_aln, scoring)
